...
if os.path.exists("subscribe_anim.mp4"):
    ...
    print("[⚠️] WARNING: 'subscribe_anim.mp4' NOT FOUND in directory. Skipping animation.")
...
# =================================================================
# 8️⃣ الرفع الآمن إلى Catbox.moe + تسجيل المخرجات بالطريقة الصحيحة لـ GitHub Actions
# =================================================================
...
for attempt in range(1, 4):
    try:
        ...
        if response.status_code == 200 and response.text.strip().startswith("http"):
            ...
            print(f"\n[🚀] SUCCESS! Direct Video URL for n8n: {direct_link}")
            break
        else:
            print(f"[❌] Upload attempt {attempt}/3 failed. Status {response.status_code}: {response.text[:200]}")
    except Exception as e:
        print(f"[❌] Exception during upload attempt {attempt}/3: {e}")
...
# --- الطريقة الحديثة الصحيحة لتمرير المخرجات داخل GitHub Actions ---
# صيغة "::set-output name=...::" القديمة معطّلة على المشغّلات الحالية ولن تُسجّل أي قيمة فعلياً.
github_output_path = os.environ.get("GITHUB_OUTPUT")
if github_output_path:
    with open(github_output_path, "a") as gh_out:
        gh_out.write(f"video_url={direct_link}\n")
else:
    print("[⚠️] GITHUB_OUTPUT env var not found — لا يبدو أن هذا التشغيل داخل GitHub Actions.")
